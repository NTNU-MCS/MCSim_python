...
def plot_test(data,test):
    ...
    [long,lat] = interpolate_sampling(long,lat)
    ...
    plt.plot(x,y,'-')
    plt.axis('equal')
    plt.title('Trajectory: '+test['description']+', '+dt_start+'-'+dt_end)
    plt.xlabel('East [m]')
    plt.ylabel('North [m]')
    ...
    for i,channel in enumerate(test['channels']):
        ...
        plt.figure(i+2)
        plt.plot(df.timestamp-t0,df.value)
...
channels = ['SpeedKnots','Heading','YawRate','Roll','Pitch','port_AzimuthOrder',
            'stbd_RPMOrder','port_RPMOrder','LonGroundSpeed','TransGroundSpeed','CourseTrue','Wind_Speed']
...
plot_test(data,test21)
#plot_test(data,test9)

#Speed versus RPM

Speed = np.array([7.3, 10.5, 12])/1.9834
RPM = [109, 170, 218]

plt.figure()
plt.plot(RPM,Speed,'-o')
