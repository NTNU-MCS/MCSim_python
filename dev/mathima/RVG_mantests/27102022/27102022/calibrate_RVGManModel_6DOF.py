...
par_path = str(Path(os.path.dirname(__file__)).parents[4])  
...
def RPMtoSpeed(RPM):
    ...
    SpeedData = np.array([7.3, 10.5, 12])/1.9834
    RPMdata = [109, 170, 218]
    f = interpolate.interp1d(RPMdata, SpeedData,fill_value='extrapolate')
    Speed = f(RPM)
...
revs_d=np.linspace(109,218,len(tvec))

thrust_state = np.array([0,109]) #azimuth angle and revs
nu=np.array([3.68,0,0,0,0,0])
...
plt.figure(8)
revs = x_out[13]
speed_meas = RPMtoSpeed(revs)
plt.plot(tvec,speed_meas)
