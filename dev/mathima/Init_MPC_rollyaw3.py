...
for j in range(0,M):
    ...
    U0 = Ustart+j*(Ustop-Ustart)/M
    U1 = Ustart+(j+1)*(Ustop-Ustart)/M
    [k1, k1_q] = f(X, U0);
    [k2, k2_q] = f(X + DT/2 * k1, U0+(U1-U0)*DT/2)
    [k3, k3_q] = f(X + DT/2 * k2, U0+(U1-U0)*DT/2)
    [k4, k4_q] = f(X + DT * k3, U1)
    X=X+DT/6*(k1 +2*k2 +2*k3 +k4)
    Q = Q + DT/6*(k1_q + 2*k2_q + 2*k3_q + k4_q)

F = Function('F', [X0, Ustart,Ustop], [X, Q], ['x0','u0','u1'], ['xf', 'qf'])
...
for i in [1]:
    ...
    sol = S(x0=w0,lbx=lbw,ubx=ubw,lbg=-ubg,ubg=ubg,p=vertcat(x0,u0))
...
for iCase in [0]:
    ...
    for j in range(1):    
        ...
        for i in range(SimN):
              ...
              sol = S(x0=np.append(tmp[1:N],tmp[N-1:]),lbx=lbw,ubx=ubw,lbg=lbg,ubg=ubg,p=vertcat(x,u0))
              toc = time.time()
              solvetime[i]=toc-tic
