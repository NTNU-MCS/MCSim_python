...
def dot_RVG_Man_3DOF_lq(x,u,w,parV,parA,parS):
    ...
    rt2 = parA['rt2'] #thruster location
    rt1 = parA['rt1'] #thruster location
    ...
    u_t1 = nu[0]-nu_c[0]-nu[2]*rt1[1] #surge velocity at thruster location
    v_t1 = nu[1]-nu_c[1] +nu[2]*rt1[0]#sway velocity at thruster location 


    Fx1, Fy1 = th.RVGazimuth_man(u_t1,v_t1,azi,revs) 
    ForceAct1 = np.array([Fx1,Fy1,Fy1*rt1[0]-Fx1*rt1[1]])        
    
    u_t2 = nu[0]-nu_c[0]-nu[2]*rt2[1] #surge velocity at thruster location
    v_t2 = nu[1]-nu_c[1] +nu[2]*rt2[0]#sway velocity at thruster location 


    Fx2, Fy2 = th.RVGazimuth_man(u_t2,v_t2,azi,revs) 
    ForceAct2 = np.array([Fx2,Fy2,Fy2*rt2[0]-Fx2*rt2[1]])       
    
    ForceAct = (ForceAct1+ForceAct2)
...
def dot_RVG_Man_6DOF_lq(x,u,w,parV,parA,parS):
    ...
    azi = x[12]
    revs = x[13]
#    rt = parA['rt'] #equivalent thruster location
...
#    Fx, Fy = th.RVGazimuth_man(u_t,v_t,azi,revs) 
#    ForceAct = np.array([Fx,Fy,0,-Fy*rt[2],-Fx*rt[0],Fy*rt[0]])    

    #two thruster model
    u_t1 = nu[0]-nu_c_3[0]-nu[5]*rt1[1] #surge velocity at thruster location
    v_t1 = nu[1]-nu_c_3[1] +nu[5]*rt1[0]#sway velocity at thruster location 


    Fx1, Fy1 = th.RVGazimuth_man(u_t1,v_t1,azi,revs) 
    ForceAct1 = np.array([Fx1,Fy1,0,-Fy1*rt1[2],-Fx1*rt1[0],Fy1*rt1[0]-Fx1*rt1[1]])        
    
    u_t2 = nu[0]-nu_c_3[0]-nu[5]*rt2[1] #surge velocity at thruster location
    v_t2 = nu[1]-nu_c_3[1] +nu[5]*rt2[0]#sway velocity at thruster location 


    Fx2, Fy2 = th.RVGazimuth_man(u_t2,v_t2,azi,revs) 
    ForceAct2 = np.array([Fx2,Fy2,0,-Fy2*rt2[2],-Fx2*rt2[0],Fy2*rt2[0]-Fx2*rt2[1]])       
    print(ForceAct2[4])
    ForceAct = (ForceAct1+ForceAct2)
    ...
    Fuuu = parV['duuu']*(nu[0]-nu_c_3[0])**3
    F4u = parV['d4u']*(nu[0]-nu_c_3[0])**4
    
    F=Fvv+ForceAct+w+np.array([-Fuuu-F4u,0,0,0,0,0])
#    print(F)
...
def dot_RVG_Man_4DOF_lq(x,u,w,parV,parA,parS):
    """Time derivative of RVG 4DOF vessel model, moving in uniform
    and steady currents. Linear stiffness matrix, consistent with small angle
    assumption in roll. Uses a linear+quadratic damping formulation. 
    Uses the thrusters.RVGazimuth_man function, with the two RVG azimuth 
    thrusters operated with the same thrust and azimuth angle. 
    Thruster states are azimuth angle and propeller revs. 
    Control input is desired thruster states, while disturbance input is 
    body-fixed forces (e.g. wind and wave loads). 
    Input: 
