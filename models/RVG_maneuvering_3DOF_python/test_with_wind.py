...
if __name__ == '__main__':
    ...
    """
    Condition of this verification study
        - Wind blows from 90deg to the north.
            - beta_w = 270 (deg) in Fossen's definition.
            - beta_w = 90 (deg) in Vico definition.
        - Wind speed is 10.0 (m/s).
        - Azimuth thruster angles are zero.
        - Azimuth thruster revolutions are 100 (rpm).
    """

    # Vico result: calculated by NTNU Ålesund with same conditions.
    np_load = np.load('Data_vico/straight_with_wind.npz')
    N_vico = np_load['N']
    E_vico = np_load['E']
    ...
    ax.plot(Log_states[:-1, 1], Log_states[:-1, 0], color='black', label='python')
    ax.plot(E_vico[:nph], N_vico[:nph], color='blue', label='vico')
    ...
    ax.plot(Log_states[:-1, 3], color='black', label='Python')
    ax.plot(u_vico[:nph], color='blue', label='vico')
    ax.set_ylabel('u(m/s)')
    plt.legend()
    ax = fig.add_subplot(312)
    ax.plot(Log_states[:-1, 4], color='black', label='Python')
    ax.plot(v_vico[:nph], color='blue', label='vico')
    ax.set_ylabel('v(m/s)')
    plt.legend()
    ax = fig.add_subplot(313)
    ax.plot(np.rad2deg(Log_states[:-1, 5]), color='black', label='Python')
    ax.plot(rdeg_vico[:nph], color='blue', label='vico')
